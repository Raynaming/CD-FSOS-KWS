...
import numpy as np
from classifiers.prototype_reprojection import get_reproj_dist
...
class NearestClassMean(nn.Module):
    ...
    def __init__(self, backbone=None, cuda=True, reprojection=False):
        """
        ...
        self.recalibration = reprojection
        ...
        if hasattr(self.backbone.criterion, 'distance'):
            if self.backbone.criterion.distance == 'euclidean': 
                if self.recalibration:
                    scores = get_reproj_dist(X, self.muK.unsqueeze(1), self.beta)
                else:
